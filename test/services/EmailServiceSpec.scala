...
class EmailServiceSpec extends SpecBase with Generators with ScalaCheckPropertyChecks with BeforeAndAfterEach {
  ...
  val reportingEntityName = "ReportingEntityName"
  val reportingStartDate  = DateTimeFormatUtil.displayFormattedDate(LocalDateTime.now)
  val reportingEndDate    = DateTimeFormatUtil.displayFormattedDate(LocalDateTime.now)
  val reportTypeContent   = "The file contains Test data."

  val agentDetails =
    AgentContactDetails("ARN", AgentResponseDetail(subscriptionId, Option(tradingName), isGBUser = true, agentPrimaryContact, Some(agentSecondaryContact)))
  val agentSingleContactDetails =
    AgentContactDetails("ARN", AgentResponseDetail(subscriptionId, Option(tradingName), isGBUser = true, agentPrimaryContact, None))
  ...
  "Email Service" - {
    "sendAndLogEmail" - {
      "must submit to the email connector with valid details and return ACCEPTED and NO_CONTENT for a Primary contact only" in {
        ...
        val result = emailService.sendAndLogEmail(subscriptionId,
                                                  submissionTime,
                                                  messageRefId,
                                                  None,
    ...
    "sendEmail" - {
      ...
      "must submit to the email connector when 1 set of valid details provided" in {
        ...
        val result = emailService.sendEmail(subscriptionId,
                                            submissionTime,
                                            messageRefId,
                                            None,
                                            isUploadSuccessful = true,
      ...
      "must submit to the email connector once for Agent only when Agent and client both have 1 set of valid details provided for failed file upload" in {
        ...
        val result =
          emailService.sendEmail(
            ...
            submissionTime,
            messageRefId,
            Some(agentSingleContactDetails),
    ...
    "getReportTypeMessage" - {
      ...
      "mus give correct message for respective Report Type" in {
        ...
        emailService.getReportTypeMessage(
          DeletionOfAllInformation
        ) mustBe "The file contains a deletion of all previously reported information for this reporting period."
